...
- **归属判断**：提炼前先看 `Atoms/` 下是否已有可复用的原子，避免重复创建（可以直接列出 `Atoms/` 目录扫一遍标题）
...
- **每次沉淀都做全量比对**：不要因为怕麻烦只看本次接触到的卡——Atoms/ 数量少增长慢，Pattern 卡的 Key Insight 也就一两段，全量扫一遍对 agent 而言开销很低，没有理由省略
