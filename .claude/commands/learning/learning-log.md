...
3. **Check if this week's log exists** at `Learning/Plans/<CODE>/Weeks/<YYYY-WXX>.md`:
   - If it exists: show its contents and ask if the user wants to update it
   - If not: create it

4. **When creating**, read `Learning/Plans/<CODE>/00_plan.md` to extract current phase goals and pre-fill the "本周目标" section.
...
- If `Learning/Plans/<CODE>/00_plan.md` doesn't exist, prompt the user to run `/learning-init` first
