...
This runs the hybrid Python + Anthropic API pipeline defined in `scripts/cc-plugins/run.sh`:
  Step 0: Python fetches plugin repos via GitHub Search + npm Registry
  Step 1: Anthropic API classifies (is_plugin gate), scores (4 dimensions), and writes bilingual summaries
...
1. Run the pipeline:
   ...
   - Typical runtime: ~60-120 seconds (GitHub search + npm lookups + Anthropic API calls).
...
2. Once the user confirms the pipeline finished, check the result:
   ...
   - **Failure**: Show the error output and suggest checking `ANTHROPIC_API_KEY`, network connectivity, or GitHub API rate limits.
