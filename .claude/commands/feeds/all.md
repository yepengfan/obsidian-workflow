...
Both pipelines are independent and idempotent.

## Instructions

1. **Check modules**: Read `system/modules/feeds-ai-digest/module.md` and `system/modules/feeds-github-trending/module.md`. Note which are enabled. If BOTH are disabled → reply "⛔ Both feed modules are disabled. Enable them via `/module-toggle`." and STOP.
...
2. **Run enabled pipelines in parallel** — launch a subagent for each enabled feed:
   ...
   Run both subagents concurrently (`run_in_background=false`, send both Agent calls in one message). If only one module is enabled, just run that one.

3. **Report results** — after both complete, give a unified summary:
