...
2. **已有 pattern → 更新**:
   ...
   - 根据本次做题过程自动重新计算 `confidence`（规则同下方新建 pattern），取 **当前值与本次值的较大者**（confidence 只升不降，降级靠 review 时重做验证）
...
3. **新 pattern → 创建**:
   ...
   - `confidence`: 基于本次做题过程自动推断（不要问用户）:
     - **5**: 独立解出（仅用 L1），代码审核无 bug
     - **4**: 独立解出（仅用 L1-L2），代码有小问题但思路正确
     - **3**: 需要方向提示（到 L3）才解出
     - **2**: 需要 Pseudocode（到 L4）或看提示后才写出代码
     - **1**: 放弃 / 看完整答案
