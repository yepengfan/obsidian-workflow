...
navToday.addEventListener("click", async () => {
  ...
  const toolbarCode = [
    ...
    '  }',
    '  // Collect task groups (top-level task + indented children)',
    '  const groups = [];',
    '  const before = []; // blank lines before first task',
    '  const after = [];  // trailing blank lines',
    '  let cur = null;',
    ...
    '      if (indent === 0) {',
    '        cur = { lines: [line], rank: 4 };',
    '        for (const [emoji, r] of Object.entries(rank)) {',
    '          if (trimmed.includes(emoji)) { cur.rank = r; break; }',
    '        }',
    '        groups.push(cur);',
    '      } else if (cur) {',
    '        cur.lines.push(line);',
    '      }',
    '    } else if (trimmed === "" && groups.length === 0) {',
    '      before.push(line);',
    '    } else if (trimmed === "") {',
    '      after.push(line);',
    '    } else if (cur) {',
    ...
    '  const sorted = [...before];',
    '  for (const g of groups) sorted.push(...g.lines);',
    '  sorted.push(...after);',
