...
- **scripts/** — Vault automation scripts.
  - `ai-digest/` — Bedrock-powered RSS digest pipeline (Python). Fetches 92 Karpathy-curated feeds → dedup → Haiku scoring → Sonnet summarization → Obsidian markdown reports. Run via `python -m digest` from `scripts/ai-digest/`. Publishes cost metrics to CloudWatch.
