...
- **Archive/** — Archived/legacy content. Contains `Training/` (legacy learning resources), `Note/` (old notes), `Entertainment/` (hobbies, e.g., Mahjong reference notes).
...
- **Learning/** — Structured learning plans and book reading. Each plan lives in a subfolder named by its code (e.g. `Learning/AISA/`). Contains `00_plan.md` (goals, phases, timeline), `00_map.md` (concept map), `Weeks/` (weekly logs), `Courses/`, `Projects/`, and `Attachments/` (plan-specific media). The folder name is the plan identifier — used as shorthand in all commands (`/learning-log AISA`). Also contains `Books/` (book reading workflow — see `Learning/Books/CLAUDE.md`). Managed via `/learning/learning-init`, `/learning/learning-log`, `/learning/learning-review`.
- **Reading/** — Article notes from external sources. `Matter/` (Matter app articles), `Instapaper/` (saved article highlights from Instapaper).
