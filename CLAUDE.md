...
- **Feeds/** — Auto-generated content feeds. `AI-Daily/` contains daily AI news digests (中英文) generated by Shell Commands plugin on Obsidian startup.
- **scripts/** — Vault automation scripts.
  - `ai-digest/` — Hybrid Python + Claude Code RSS digest pipeline. Python fetches 92 Karpathy-curated feeds and deduplicates (`fetch.py`), then Claude Code CLI scores, summarizes bilingually, and writes Obsidian markdown reports (`digest-prompt.md`). Run via `bash scripts/ai-digest/run.sh`.
